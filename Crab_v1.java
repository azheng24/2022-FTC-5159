...
public class Crab_v1 {
    ...
    */
    private double gdNavXOutput;
    //private ElapsedTime runtime = new ElapsedTime();

    //Kauai Labs navX constants
    private final byte NAVX_DEVICE_UPDATE_RATE_HZ = 50;
    private final double TARGET_ANGLE_DEGREES = 0.0d;
    //private final double TOLERANCE_DEGREES = 2.0d; //when within this tolerance, no more heading
    ...
    //if in telop, and robot does not quite move with respect to feild
    //  increase P
    private final double YAW_PID_P = 0.07d; //.030 was okay
    private final double YAW_PID_I = 0.00d; //.0012 is good
    ...
    //select library
    private final boolean NAVX_KL_LIB=false;
    ...
    private final double TANK_FAST_PWR = 1.0d;  //fastest pwr for tank drive
    private final double TANK_SLOW_PWR = .5d;  //slowest pwr for tank drive
    ...
    private double TANK_LEFT_ADJ=-.10d; //increase this sends tank left down
    private double TANK_RIGHT_ADJ=-.05d; //increase this sends tank right down
    private double TANK_FWD_ADJ=.05d; //increast this sends tank right
    //private double TANK_REV_ADJ=-.12d; //increase this sends tank right as it reverses
    private double TANK_REV_ADJ=-.12d; //increase this sends tank right as it reverses

    //Omni drive constants
    private final double OMNI_FAST_PWR = 1.0; //fastest pwr for omni drive
    private final double OMNI_SLOW_PWR = .5;  //slowest pwr for omni drive
    ...
    public final int CCW = -1, CW = 1, NOSPIN = 0, SPINTOZERO = 100;
    ...
    //drive constants
    private final double MOMENTUM_ADJ=8d;  //rotate robot, see if jump, adjust this
    ...
    //auton constants: states for op mode
    private final int AUTONINIT = 0;
    private final int AUTONEXECUTE1 = 120;
    private final int AUTONEXECUTE2 = 130;
    private final int AUTONDONE = 1000;

    private final double AUTONDIRWRMIN =.5d; //increase this number until you see a "jerk" in auton movements
    ...
    private double gdMomentumAdjustmentDeg = 0d, gdTankMomentumAdjustmentDeg = 0d;
    ...
    private double gdRotationPwrRatioAdj = (double) .85;

    private final int TIMEINIT = -9999;
    private int gnAutonState = AUTONINIT;
    private long glAutonTimeStamp, glAutonTimeStart = TIMEINIT;
    private int gnEncoderCntTarget = 0, gnSlowDownTarget=0,gnEncoderStart;
    ...
    private long glDIYYawTimeLast = 0;
    private double gdDIYPPwr = 0d, gdDIYIPwr = 0d;
    private double gdDIYYawDistLast = 0d;
    private int gnDIYYawSpin = NOSPIN;

    //Autonomous Constants
    final int AUTON_FWD = 0;
    final int AUTON_REV = 1;
    final int AUTON_LEFT = 2;
    final int AUTON_RIGHT = 3;

    //Autonomous Public Constants
    public final int INTERRUPTED = -1;
    public final int TIMEDOUT = -2;
    public final int ERR = -100;
    public final int DONE = 0;
    public final int INPROGRESS = 1;
    public final int LINEFOUND = 5;

    //Color sensor globals
    private final int LF=1,LB=3,RF=2,RB=4;
    private final int AUTONSLOWDOWNDIST =100;
     ...
     * @param dRobotWeightLbs      the weight of the robot in lbs. affects momentum. start with 25
     *                             Decrease if robot rotates more after initially stopping
     *                             Decrease if robot rotates back after initially stopping
     * @param dRotationPwrRatioAdj increase to add more rotational power compared to directional pwr
     *                             start with .67
     *                             <p>
     *                             <p>
     *                             Note: incorrect requested layout defaults to CHASSIS_LEFT_FWD
     */
    public void initialize(OpMode opMode, int nReqLayout, String strMtrLFName, String strMtrLBName,
                           String strMtrRFName, String strMtrRBName, String strNavXName,
                           double dRobotWeightLbs,  double dRotationPwrRatioAdj) {
        opMode.telemetry.addData("Crab","Initializing...");
        opMode.telemetry.addData("Crab","    Current robot direction will be polar 0");
        ...
        gdRotationPwrRatioAdj = dRotationPwrRatioAdj;
        calcMomentumAdjustmentDeg();
        opMode.telemetry.addData("Crab","   Complete");
     ...
     * @param opMode opmode calling this method
     */
    void closeGraceful(OpMode opMode) {
    ...
    private boolean initNavX(OpMode opMode, String strNavXName) {
        //setup nav
        if(NAVX_KL_LIB)
           opMode.telemetry.addData("Crab", "   KL " + strNavXName);
        else
            opMode.telemetry.addData("Crab", "   QC " + strNavXName);
    ...
    public void operate(OpMode opMode, Gamepad gamepad) {
        ...
        chkPreDefBtns(gamepad);
        chkResetRobotYawZero(gamepad);
        //send telemetry to drive;r station (no robot control)
        if (getControlMode() ==OMNI_CONTROL) {
            opMode.telemetry.addData("Crab", "Omni Mode-Direction Stick Polar%.2f",
                   getDirectionStickPolarAngle());
            opMode.telemetry.addData("Crab", "   Omni full pwr %.2f MomentAdj %.2f",
                    getOmniPwr(), getMomentumAdj());
        } else if (getControlMode() == TANK_CONTROL) {
            opMode.telemetry.addData("Crab", "Tank Mode-LeftStick Y:%.2f RightStick Y:%.2f",
                    gamepad.left_stick_y, gamepad.right_stick_y);
            opMode.telemetry.addData("Crab", "   %s Pwr %.2f", getTankMovement(),
                    getTankPwrMagnitude());
        }
        opMode.telemetry.addData("Crab","NavX-%s yaw:%.1f target:%.1f", getNavXStatus(),
                getNavXYaw(),getYawAngleToHold());

        opMode.telemetry.addData("Crab", "PWR LF:%.2f RF:%.2f",
                getResultantPwr(MTR_LF), getResultantPwr(MTR_RF));
        opMode.telemetry.addData("Crab", "PWR LB:%.2f RB:%.2f",
                getResultantPwr(MTR_LB), getResultantPwr(MTR_RB));
        opMode.telemetry.addData("Crab", "Rotation Pwr %.2f", getRotationPwr());

        opMode.telemetry.addData("Crab", "ENC LF:%d RF:%d",
                getMtrCurrentPosition(MTR_LF), getMtrCurrentPosition(MTR_RF));
        opMode.telemetry.addData("Crab", "ENC LB:%d RB:%d",
                getMtrCurrentPosition(MTR_LB), getMtrCurrentPosition(MTR_RB));
    }
    ...
    void chkPreDefBtns(Gamepad gamepad) {
    ...
    void setPreDefMoveToZero() {
        enableNavXYawPID(true);
        setpointNavXYaw(0);
        gdRotationPwr = 0;
        gnRobotSpin = NOSPIN;
        gbRotationStkOrBtnPressed = false; //dont really need this
    }
    ...
    private void calcMomentumAdjustmentDeg() {
        ...
        double dMaxMomentumAdj = 6; //make this bigger if robot correct CCW after a CW rotation
        ...
        //gdMomentumAdjustmentDeg = dAdjDeg;
        //compnesate for weight
        //gdMomentumAdjustmentDeg *= dAdjDeg;

         */
        gdMomentumAdjustmentDeg=MOMENTUM_ADJ; //better than the calc above!
    }
    ...
    private void addDirectionAndRotationPwr() {
        ...
        adResultantPwr[LF] = gdDirectionLFPwr + gdRotationPwr;
        adResultantPwr[RF] = (gdDirectionRFPwr) - gdRotationPwr;
        adResultantPwr[LB] = gdDirectionLBPwr + gdRotationPwr;
        adResultantPwr[RB] = gdDirectionRBPwr - gdRotationPwr;
        ...
        gdResultantLFPwr = adResultantPwr[LF];
        gdResultantRFPwr = adResultantPwr[RF];
        gdResultantLBPwr = adResultantPwr[LB];
        gdResultantRBPwr = adResultantPwr[RB];
    }
    ...
    private void setOmniRotationPwr(Gamepad gamepad, boolean bRotationStick) {
        ...
        if (!chkStickDeadZone(gamepad, bRotationStick)) {
            ...
            if (dStickX < 0) {
                gdRotationPwr = -(gdOmniPwrMagnitude * gdRotationPwrRatioAdj);
                gnRobotSpin = CCW;
        ...
        //
        //The momentum adjustment is throttle dependent and robot weight dependent
   ...
   private void chkResetRobotYawZero(Gamepad gamepadToChk) {
       ...
       if (gamepadToChk.guide) { //back button hit
           if(gnNavXStatus==NAVX_KAUAILABS_LIB) {
               ...
               gdYawAngleToHold = 0;
               yawPIDController.setSetpoint(gdYawAngleToHold);
               gbRotationStkOrBtnPressed = false;
               gdRotationPwr = 0d;
               gdDirectionLFPwr = gdDirectionLBPwr = gdDirectionRFPwr = gdDirectionRBPwr = 0;
                */
    ...
    private boolean setOmniCrawlPwr(Gamepad gamepad, boolean bDirStick) {
        ...
        if (bDirStick == STICK_RIGHT) {
            if (gamepad.y) {
                //apply pwr fwd
                gdDirectionLFPwr = gdOmniPwrMagnitude;
                gdDirectionRFPwr = gdOmniPwrMagnitude;
                gdDirectionLBPwr = gdOmniPwrMagnitude;
                gdDirectionRBPwr = gdOmniPwrMagnitude;
    ...
    public boolean isNavXConnected() {
        ...
        if (gnNavXStatus == NAVX_KAUAILABS_LIB) {
            ...
            dont forget to delete the line below */
            return true;
    ...
    public double getNavXCorrectionPwr() {
        ...
        if (gnNavXStatus == NAVX_KAUAILABS_LIB) {
            ...
            if (yawPIDController.isNewUpdateAvailable(yawPIDResult)) {
                //telemetry.addData("NavX update Avail","");
                if (yawPIDResult.isOnTarget()) {
                    return 0d;
                } else {
                    double output = yawPIDResult.getOutput();
                    gdNavXOutput=output;
                    //telemetry.addData("   Not Ontarget","%.2f",output);
                    return output;
                }
            } else {
                ...
                //telemetry.addData("NavX update NOT Avail","");
                //telemetry.addData("   Last value","%.2f",dLast);
                return gdNavXOutput;
            }
            dont forget to delete the line below
            */
            ...
            return calcDIYYawCorrectionPwr();
        } else {
            return 0;
        }
    }
     ...
     */
    private double calcDIYYawCorrectionPwr() {
        ...
        if ((lCurrentTime - glDIYYawTimeLast) > CCYAW_TIMEOUT) {
            glDIYYawTimeLast = lCurrentTime;
            gdDIYIPwr = 0;
            gdDIYPPwr = 0;
            gdDIYYawDistLast = 0d;
            gnDIYYawSpin = NOSPIN;
        ...
        //   the larger the P
        gdDIYPPwr = CCYAW_P * dDistToRotate;
        ...
        //
        if (gnDIYYawSpin == CCW) { //robot spin is CCW
            ...
            if (dDistToRotate < 0) {
                ...
                if (gdDIYYawDistLast < dDistToRotate) {
                    // -180---------------0
                    //          | Current |
                    //      !    last     |
                    //
                    ...
                    dCalc = CCYAW_I * ((double) lCurrentTime - (double) glDIYYawTimeLast); //magnitude of correction
                    gdDIYIPwr -= dCalc; //reduce correction by adding since initial spin is CCW
                } else {
                    // -180---------------0
                    //     |   Current    |
                    //          !  last   |
                    ...
                    dCalc = CCYAW_I * ((double) lCurrentTime - (double) glDIYYawTimeLast); //magnitude of correction
                    gdDIYIPwr += dCalc; //increase correction subtracting since initial spin is CW
                }
                ...
                dCalc = CCYAW_I * ((double) lCurrentTime - (double) glDIYYawTimeLast); //magnitude of correction
                gdDIYIPwr -= dCalc; //correction by adding
            }
        } else if (gnDIYYawSpin == CW) { //robot spin is CCW
            //if got here, know that the correction Dist initially set was neg or CCW
            //check if the current correction distance is pos or neg
            if (dDistToRotate > 0) {
                //the distance to currently rotate is also neg
        ...
        glDIYYawTimeLast = lCurrentTime;
        gdDIYYawDistLast = dDistToRotate;
    ...
    private void setDIYSetPoint(double dPolarAngle) {
        ...
        gdDIYYawDistLast = calcDistToRotateDegrees(dTarget, dCurrent);
        if (gdDIYYawDistLast < 0) gnDIYYawSpin = CCW;
        else if (gdDIYYawDistLast > 0) gnDIYYawSpin = CW;
        else gnDIYYawSpin = NOSPIN;
        glDIYYawTimeLast = System.currentTimeMillis();
        gdDIYIPwr = gdDIYPPwr = 0d;
        */
    ...
    /**
     * Get the I power from the DIYYawCorrection
     *
     * @return gdDIYIPwr
     */
    public double getDIYIPwr() {
        return gdDIYIPwr;
    }

    /**
     * Get the P power from the DIYYawCorrection
     *
     * @return gdDIYPPwr
     */
    public double getDIYPPwr() {
        return gdDIYPPwr;
    }
 ...
 /*X
        ...
        return true;
X  dont forget to delete line below*/
       return false;
    }
     ...
     * @return DCMotor current position
     */
    ...
    /**
     * gets DcMotor's getTargetEncoder Count()
     *
     * @return target encoder count for the LF motor
     */
    public int getMtrTargetEncCnt() {
        return gnEncoderCntTarget;
    }
    /**
     * gets DcMotor
   ...
   public int autonpivotTurnNavXLOp(LinearOpMode linopMode, double dTargetHeading, long lMaxTime_msec) {
       ...
       while (linopMode.opModeIsActive() && !linopMode.isStopRequested() &&
           ...
           linopMode.telemetry.addData("Direction", "Target:%.2f Current %.2f", dTargetHeading, getNavXYaw());
           linopMode.telemetry.addData("Yaw Pwr", "%.2f P:%.2f I:%.2f dist:%.2f", gdRotationPwr, gdDIYPPwr, gdDIYIPwr, gdDIYYawDistLast);
   ...
   private boolean isOnTarget() {
       if (gnNavXStatus == NAVX_KAUAILABS_LIB) {
           ...
           dont forget to delete line below  */
           return false;
       }
       else {
          if(Math.abs(gdYawAngleToHold-getNavXYaw())>TOLERANCE_DEGREES) return true;
          else return false;
       }
   }
    ...
    public int autonmoveNavXLOp(LinearOpMode opMode, int nDistToMoveEncoderCount, long lMaxTime,
   ...
   private  void autonmoveSetDirPwr(double dPwrAbs,int nDir,int nEncCntTravelled, int nEncCntRemain,boolean bSlowDown) {
       int nPwrEncCalc = Math.round((float) dPwrAbs * 100f); //Ex: dPwrAbs=.66 pwrEncCalc is 66
       int nTravelled=Math.abs(nEncCntTravelled);
       long lEncCntBase=Math.round(AUTONDIRWRMIN);

       //start increasing power when start moving
       //Example AUTONDIRPWRBASE=.25 so lEncCntBAse=25
       //0-25 encoder count, use .25 power all the time
       if((long)nTravelled<=lEncCntBase) { //constant needed??
           gdDirectionLFPwr = autonmoveGetDirAdjForWheel(LF,nDir)* AUTONDIRWRMIN;
           gdDirectionRFPwr = autonmoveGetDirAdjForWheel(RF,nDir)* AUTONDIRWRMIN;
           gdDirectionLBPwr =  autonmoveGetDirAdjForWheel(LB,nDir)* AUTONDIRWRMIN;
           gdDirectionRBPwr =  autonmoveGetDirAdjForWheel(RB,nDir)* AUTONDIRWRMIN;
           return;
       }
       //if got here travelled at least  25 encoder counts
       //ramp up power until get to 100 encoder counts
       //Example AUTONDIRPWRMIN=.25 so traveled at least 25 encoder counts
       //25 to limit, increase power until reach dPwrAbs
       if((nTravelled<100)&&(nTravelled<=nPwrEncCalc)) { //use power proportional to dist travelled
           gdDirectionLFPwr = autonmoveGetDirAdjForWheel(LF,nDir)*((double)nTravelled/100d);
           gdDirectionRFPwr = autonmoveGetDirAdjForWheel(RF,nDir)*((double)nTravelled/100d);
           gdDirectionLBPwr =  autonmoveGetDirAdjForWheel(LB,nDir)*((double)nTravelled/100d);
           gdDirectionRBPwr =  autonmoveGetDirAdjForWheel(RB,nDir)*((double)nTravelled/100d);
           return;
       }


       // check if need to slowdown at end
       if(bSlowDown) {
           //start decreasing power... but don't go below base power needed to move
           if ((nEncCntRemain < nPwrEncCalc)&&((long) nEncCntRemain > lEncCntBase)) {
               double dRemainPct = (double) nEncCntRemain / 100d;
               gdDirectionLFPwr = autonmoveGetDirAdjForWheel(LF, nDir) * dRemainPct;
               gdDirectionRFPwr = autonmoveGetDirAdjForWheel(RF, nDir) * dRemainPct;
               gdDirectionLBPwr = autonmoveGetDirAdjForWheel(LB, nDir) * dRemainPct;
               gdDirectionRBPwr = autonmoveGetDirAdjForWheel(RB, nDir) * dRemainPct;
               return;
           }
           //only decrease power so much
           if ((long) nEncCntRemain <= lEncCntBase) {
               gdDirectionLFPwr = autonmoveGetDirAdjForWheel(LF, nDir) * AUTONDIRWRMIN;
               gdDirectionRFPwr = autonmoveGetDirAdjForWheel(RF, nDir) * AUTONDIRWRMIN;
               gdDirectionLBPwr = autonmoveGetDirAdjForWheel(LB, nDir) * AUTONDIRWRMIN;
               gdDirectionRBPwr = autonmoveGetDirAdjForWheel(RB, nDir) * AUTONDIRWRMIN;
       ...
       //if got here, between the start and end of the range travelling
       gdDirectionLFPwr = autonmoveGetDirAdjForWheel(LF,nDir)*dPwrAbs;
       gdDirectionRFPwr =  autonmoveGetDirAdjForWheel(RF,nDir)*dPwrAbs;
       gdDirectionLBPwr =  autonmoveGetDirAdjForWheel(LB,nDir)*dPwrAbs;
       gdDirectionRBPwr =  autonmoveGetDirAdjForWheel(RB,nDir)*dPwrAbs;
       return;
   }

   private double autonmoveGetDirAdjForWheel(int nWheel, int nDir)  {
       switch (nDir) {
           case AUTON_FWD:
               switch (nWheel) {
                   case LF:
                       return 1d+TANK_FWD_ADJ;
                   case RF:
                       return 1d-TANK_FWD_ADJ;
                   case LB:
                       return 1d+TANK_FWD_ADJ;
           ...
           case AUTON_RIGHT:
               switch (nWheel) { //pos adj sends down right; neg adj sends up right
                   case LF: // ^
                       return 1d+TANK_RIGHT_ADJ;
                   case RF: // v
                       return -1d-TANK_RIGHT_ADJ;
                   case LB: // v
                       return -1d+TANK_RIGHT_ADJ;
           ...
           case AUTON_LEFT:
               switch (nWheel) { //pos adj sends down left; neg adj sends up left
                   case LF: // v
                       return -1d-TANK_LEFT_ADJ;
                   case RF: // ^
                       return 1d+TANK_LEFT_ADJ;
                   case LB: // ^
                       return 1d-TANK_LEFT_ADJ;
           ...
           case AUTON_REV:
               switch (nWheel) { //pos adj sends tank right; neg adj sends tank left
                   case LF:
                       return -1d-TANK_REV_ADJ;
                   case RF:
                       return -1d+TANK_REV_ADJ;
                   case LB:
                       return -1d-TANK_REV_ADJ;
